...
print("Texto:", amazon_dataset['train']['text'][:3])
print("Labels:", amazon_dataset['train']['label'][:3])

# OPÇÃO 1: Dividir o conjunto de treino em treino + validação (80/20)
from datasets import DatasetDict

# Dividindo o dataset de treino
train_indices = list(range(len(amazon_dataset['train'])))
...
train_split = amazon_dataset['train'].select(train_idx)
val_split = amazon_dataset['train'].select(val_idx)

# Criando um novo dataset com train, validation e test
amazon_dataset_with_val = DatasetDict({
    'train': train_split,
    'validation': val_split,
    'test': amazon_dataset['test']
})


print(f"\nNovo dataset com validação:")
print(f"Train: {len(amazon_dataset_with_val['train'])} exemplos")
print(f"Validation: {len(amazon_dataset_with_val['validation'])} exemplos") 
print(f"Test: {len(amazon_dataset_with_val['test'])} exemplos")

# Verificar distribuição das classes
import numpy as np
print(f"\nDistribuição de classes no treino: {np.bincount(amazon_dataset_with_val['train']['label'])}")
...
for countDataset in range(0, len(datasets)):
    ...
    for bertModel in models:
        ...
        bertModel.store_logits(test_dataset, f"amazonpolarity_test_{bertModel.model_name.split('/')[-1]}")
        bertModel.store_embeddings_only(test_dataset, f"amazonpolarity_test_{bertModel.model_name.split('/')[-1]}")
        
        # Para o conjunto de treino
        bertModel.store_logits(train_dataset, f"amazonpolarity_train_{bertModel.model_name.split('/')[-1]}")
        bertModel.store_embeddings_only(train_dataset, f"amazonpolarity_train_{bertModel.model_name.split('/')[-1]}")
        
        # Para o conjunto de validação
        bertModel.store_logits(validation_dataset, f"amazonpolarity_val_{bertModel.model_name.split('/')[-1]}")
        bertModel.store_embeddings_only(validation_dataset, f"amazonpolarity_val_{bertModel.model_name.split('/')[-1]}")
