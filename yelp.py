...
for countDataset in range(0, len(datasets)):
    ...
    for bertModel in models:
        ...
        bertModel.store_embeddings_only(test_dataset, f"{datasetsNames[countDataset]}_test_{bertModel.model_name.split('/')[-1]}")
        bertModel.store_embeddings_only(train_dataset, f"{datasetsNames[countDataset]}_train_{bertModel.model_name.split('/')[-1]}")
