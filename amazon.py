...
class GenericEncoderModel:
    ...
    def store_predictions(self, dataset, predictions, output_csv_path):
        ...
        with open(output_csv_path, mode='w', newline='') as file:
            ...
            writer.writerow(['prediction', 'label', 'title', 'content']) 
            for title, content, label, prediction in zip(dataset['title'], dataset['content'], dataset['label'], predictions):
                writer.writerow([prediction, label, content, title])
    ...
    def evaluate(self, test_dataset, dataset_name):
        ...
        self.store_predictions(self.trainer.eval_dataset, predictions, output_csv_path=f"predictions_{self.model_name}_{dataset_name}_.csv")
...
def preprocess_function(examples, tokenizer, title_key='title', content_key='content'):
