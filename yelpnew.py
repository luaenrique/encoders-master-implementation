...
class GenericEncoderModel:
    ...
    def evaluate(self, test_dataset, dataset_name):
        ...
        # Usa o dataset processado (não tem mais 'text')
...
# ========== CONFIGURAÇÃO PARA O DATASET YELP ==========
...
datasetsNames = ['yelp']
numLabels = [5]  # 5 classes (1-5 estrelas)
...
print(f"Starting Yelp experiment at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
for countDataset in range(0, len(datasets)):
    ...
    for bertModel in models:
        ...
        bertModel.store_logits(test_dataset_processed, f"test_{datasetsNames[countDataset]}")
        bertModel.store_logits(train_dataset_processed, f"train_{datasetsNames[countDataset]}")
        
        print("Logits extraction completed!")
        print(f"{'='*40}")
        
        # Salvando embeddings TRADICIONAIS (texto completo)
        print("\n" + "="*60)
        print("SALVANDO EMBEDDINGS TRADICIONAIS (texto completo)")
        print("="*60)
        
        bertModel.store_embeddings_only(test_dataset_processed, f"{datasetsNames[countDataset]}_test_{bertModel.model_name.split('/')[-1]}")
        bertModel.store_embeddings_only(train_dataset_processed, f"{datasetsNames[countDataset]}_train_{bertModel.model_name.split('/')[-1]}")
        
        # Salvando embeddings BASEADOS EM SENTENÇAS (média de sentenças)
        print("\n" + "="*60)
...
for model_name in model_names:
    print(f"\n{model_name}:")
    print(f"  - logits_{model_name}_test_yelp.npz")
    print(f"  - logits_{model_name}_train_yelp.npz") 
    print(f"  - embeddings_{model_name}_yelp_test_{model_name.split('_')[-1]}.npz")
    print(f"  - embeddings_{model_name}_yelp_train_{model_name.split('_')[-1]}.npz")
    print(f"  - sentence_embeddings_{model_name}_yelp_test_{model_name.split('_')[-1]}.npz")
    print(f"  - sentence_embeddings_{model_name}_yelp_train_{model_name.split('_')[-1]}.npz")
    print(f"  - detailed_timing_{model_name}_yelp.csv")
