...
class GenericEncoderModel:
    ...
    def store_logits(self, dataset, dataset_name):
        ...
        # CORREÇÃO 1: Usar trainer.predict() que é mais confiável
        predictions = self.trainer.predict(dataset)
        logits = predictions.predictions
        labels = predictions.label_ids
        
        # CORREÇÃO 2: Salvar com nome mais descritivo e usar compressão
        model_name_clean = self.model_name.replace('/', '_')
        output_file = f"logits_{model_name_clean}_{dataset_name}_2.npz"
        
        # CORREÇÃO 3: Calcular probabilidades para verificação
        if self.problem_type == "single_label_classification":
        ...
        # CORREÇÃO 4: Salvar com compressão e incluir probabilidades
        ...
        # CORREÇÃO 5: Verificações de integridade
        print(f"Logits saved to {output_file}")
        print(f"Logits shape: {logits.shape}")
        print(f"Labels shape: {labels.shape}")
        print(f"Number of samples: {len(labels)}")
        print(f"Number of classes in logits: {logits.shape[1]}")
        print(f"Expected number of classes: {self.num_labels}")
        print(f"Unique labels: {np.unique(labels)}")
        print(f"Logits range: [{logits.min():.3f}, {logits.max():.3f}]")
        
        # CORREÇÃO 6: Verificar accuracy para validar extração
        if self.problem_type == "single_label_classification":
            pred_classes = np.argmax(logits, axis=-1)
            accuracy = accuracy_score(labels, pred_classes)
            print(f"Accuracy from extracted logits: {accuracy:.4f}")
    ...
    def store_embeddings_only(self, dataset, dataset_name):
        ...
        output_file = f"embeddings_{self.model_name.replace('/', '_')}_{dataset_name}_2.npz"
        np.savez_compressed(output_file, 
...
for countDataset in range(0, len(datasets)):
    ...
    for bertModel in models:
        ...
        bertModel.store_logits(test_dataset, f"test_{datasetsNames[countDataset]}_2")
        
        bertModel.store_logits(train_dataset, f"train_{datasetsNames[countDataset]}_2")
        ...
        detailed_timing_file = f"detailed_timing_{bertModel.model_name.replace('/', '_')}_{datasetsNames[countDataset]}_2.csv"
