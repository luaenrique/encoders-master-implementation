...
for countDataset in range(0, len(datasets)):
    ...
    for bertModel in models:
        ...
        train_dataset = train_dataset.remove_columns([structure['contentKey'][0], structure['contentKey'][1]])
        ...
        bertModel.store_embeddings_only(test_dataset, f"snli_test_{bertModel.model_name.split('/')[-1]}")
        bertModel.store_embeddings_only(train_dataset, f"snli_train_{bertModel.model_name.split('/')[-1]}")
...
for model_name in model_names:
    print(f"\n{model_name}:")
    print(f"  - logits_{model_name}_test_snli.npz")
    print(f"  - logits_{model_name}_train_snli.npz") 
    print(f"  - embeddings_{model_name}_snli_test_{model_name.split('_')[-1]}.npz")
    print(f"  - embeddings_{model_name}_snli_train_{model_name.split('_')[-1]}.npz")
    print(f"  - detailed_timing_{model_name}_snli.csv")
